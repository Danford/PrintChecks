...
export const useReceiptStore = defineStore('useReceiptStore', () => {
  ...
  const calculatedTotals = computed((): ReceiptTotals => {
    ...
    const totalTax = currentReceipt.value.lineItems.reduce(
      (sum, item) => sum + (item.taxAmount || 0),
      0
    )
    const totalDiscount = currentReceipt.value.lineItems.reduce(
  ...
  function removeLineItem(itemId: string) {
    ...
    currentReceipt.value.lineItems = currentReceipt.value.lineItems.filter(
      (item) => item.id !== itemId
    )
    updateTotals()
    validateReceipt()
  ...
  function validateReceipt(): boolean {
    ...
    const lineItemsValid =
      currentReceipt.value.lineItems.length > 0 &&
      currentReceipt.value.lineItems.every(
  ...
  async function saveReceipt() {
    ...
    try {
      ...
      const existingIndex = receipts.findIndex((r: ReceiptData) => r.id === currentReceipt.value?.id)
      if (existingIndex >= 0) {
  ...
  function saveAsTemplate(
    name: string,
    description?: string,
    category: ReceiptTemplate['category'] = 'custom'
  ) {
