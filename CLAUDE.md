...
### Research Phase — 2026-03-30 (second pass)

Audit findings (0 vulns, all packages up-to-date, 1283 total passing tests across all packages):
- **Vue composable API docs severely out of sync**: all 4 docs (use-checks.md, use-vendors.md, use-bank-accounts.md, use-receipts.md) use `items` as the list state but actual returns use `checks`/`vendors`/`accounts`/`receipts`; also list nonexistent methods (`getCheck`, `getAccount`, `getFavoriteVendors`) and omit real computed properties (`currentCheck`, `amountInWords`, `isValid`, `currentVendor`, `favoriteVendors`, `vendorCount`, `defaultAccount`, `accountCount`, `hasUnsavedChanges`, `nextCheckNumber`, `validateCheck`, `isValid`, `hasLineItems`, `receiptCount`); this breaks any user copying from docs
- **85 ESLint warnings**: 84 `no-explicit-any` spread across test files and source; 1 `no-unused-vars` (unused `useData` import in docs Demo.vue); no errors
- **ImportExportView.vue (582 lines)** and **AnalyticsView.vue (305 lines)**: substantial business logic (export formats, data aggregation) untested; would require `@vue/test-utils` + jsdom environment
- **Docs Demo.vue**: `useData` imported but never used — trivial dead code
- All 0 vulnerabilities, no outdated packages, no type errors
...
- [ ] MEDIUM: Remove unused `useData` import in docs Demo.vue — 1 no-unused-vars warning, 1-line fix
- [ ] MEDIUM: Add view component tests for ImportExportView and AnalyticsView — requires adding `@vue/test-utils` + jsdom env to app vitest config; ImportExportView has encryption toggle, export format selection (JSON/CSV), import validation logic; AnalyticsView has statistics aggregation from multiple services
