...
- [ ] HIGH: Fix Vue composable API docs — use-checks.md/use-vendors.md/use-bank-accounts.md/use-receipts.md all show `items` but actual return names are `checks`/`vendors`/`accounts`/`receipts`; also list nonexistent methods and omit computed properties (currentCheck, amountInWords, isValid, favoriteVendors, defaultAccount, etc.); correct all 4 docs to match source
