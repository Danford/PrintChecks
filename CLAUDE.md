...
- [ ] HIGH: Fix amountToWords bug in printable-check-page.ts — recursive calls add ' Dollars' at every level, so any amount ≥ $1000 renders as e.g. "One Dollars Thousand Five Hundred Dollars"; fix by tracking recursion depth (add `recursive=false` param, skip ' Dollars' suffix when true) or replace with `to-words` npm package (already a dep in the app)
- [ ] MEDIUM: Add Vitest to printchecks app and test useFormatting.ts composable — 16 pure formatting functions (formatMoney, formatCurrency, formatDate, formatPhoneNumber, formatRoutingNumber, formatAccountNumber, formatCheckNumber, formatPercentage, formatFileSize, truncateText, capitalizeFirst, capitalizeWords, parseAmount, validateEmail, validatePhone, validateZip); no framework deps needed
- [ ] MEDIUM: Test Pinia stores with Vitest — check.ts (validation computed, amountInWords with to-words, template management), history.ts (filteredItems computed sorting/pagination), receipt.ts; requires `createPinia` + `setActivePinia`; add vitest + @pinia/testing to app devDeps
